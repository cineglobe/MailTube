...
For a VPS verification challenge, export a Netscape-format `cookies.txt` from a dedicated YouTube account and upload it in **Settings → System → YouTube cookies**. MailTube validates the format, stores the file with mode `0600` in its persistent data volume, and applies it to new yt-dlp jobs immediately. Treat the file as an account credential and remove it from MailTube when it is no longer required.
